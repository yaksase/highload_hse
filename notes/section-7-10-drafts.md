...
| Регион пользователя | Клиент→edge | Regional Inference→US GPU | Основной вывод |
| ------------------- | ----------- | ------------------------- | -------------- |
| US East             | ~5 мс       | ~10 мс                    | TTFT в основном определяется очередью и prefill на GPU |
| EU                  | ~10 мс      | ~80 мс                    | Ближайший edge уменьшает только client→edge и jitter; межрегиональный путь до GPU остаётся |
| APAC                | ~30 мс      | ~150 мс                   | Самый дорогой участок — backbone до US GPU, поэтому edge не даёт кратного выигрыша по TTFT |
