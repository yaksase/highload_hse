...
| Средний размер вложения | ~5 МБ | оценка по лимитам OpenAI* |
...
\* Лимиты OpenAI [(File Uploads FAQ)](https://help.openai.com/en/articles/8555545-file-uploads-faq): картинки до 20 МБ, документы до 512 МБ, CSV до 50 МБ, per-user cap 25 ГБ. Реальный микс в чате: ~70% фото для vision × 3 МБ (компрессированный снимок) + ~30% документов × 10 МБ (PDF/презентации) ≈ 5 МБ. Для API подавляющий объем ввода — текст, так что вложения игнорируем.
...
| Вложения | 75 × 0.07 × 5 МБ | 26.25 МБ |
| **Итого** | | **~26.5 МБ/мес** |

Вложения — **~99%** объёма пользователя.
...
| Вложения | 1 млрд × 12 × 26.25 МБ | **~315 ПБ** |
| **Итого** | | **~318 ПБ/год** |
...
| Web вложения | 57 870 × 0.07 × 5 МБ | **162.0** |
| API текст | 78 125 × 25 КБ | 15.63 |
| **Итого входящий** | | **~178.6 Гбит/с** |
...
| Входящий вложения | 28 935 × 0.07 × 5 МБ × 86 400 | **~875 ТБ** |
| Исходящий SSE (Web ~22 ТБ + API ~24 ТБ) | (28 935×9 КБ + 39 063×7 КБ) × 86 400 | ~46 ТБ |
| **Итого** | | **~1010 ТБ/сутки (~1 ПБ)** |

Вложения дают **~87%** суточного трафика. API — основной источник текстового трафика (84 ТБ против 5 ТБ у Web): полный контекст в каждом запросе.
...
* Для enterprise-клиентов с требованиями data residency поддерживается ограниченный кластер в EU [^9]
...
4. **Data residency** — для enterprise-клиентов доступен inference residency в Европе [^9]
...
Берём NGINX Plus (обоснование выбора — в §4.2). По бенчмаркам [^10][^11] аккуратно снижаем паспорт из-за реального профиля трафика (TLS 1.3 ECDSA + HTTP/2 + SSE chunked + buffering off):
...
[^9]: OpenAI. *Data Residency.* https://openai.com/enterprise-privacy/data-residency/

[^10]: Nginx Blog. *Testing Performance of NGINX Ingress Controller for Kubernetes.* https://blog.nginx.org/blog/testing-performance-nginx-ingress-controller-kubernetes

[^11]: Nginx Blog. *Testing the Performance of NGINX and NGINX Plus Web Servers.* https://blog.nginx.org/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers
