...
В качестве референса используется аудитория ChatGPT. По данным OpenAI:
- **>800 млн WAU** [^1]
- **~2.5 млрд сообщений в день** [^2]

Все расчёты ведутся для модели класса Claude Sonnet как наиболее востребованной на рынке.

### Допущения

1. **Web/API split: 85% / 15%** — официальных данных о разделении трафика между веб-чатом и API нет. Принимаем 85/15 как условность, отражающую массовость веб-аудитории.
2. **Пиковый множитель: ×2** — для глобального сервиса с распределённой по часовым поясам аудиторией.
3. **Доля вложений: 5%** web-сообщений содержат вложение. Для API — пренебрежимо мало (~0.3%).
4. **Бессрочное хранение** истории чатов для веб-пользователей. API — stateless, хранилище не требуется.
5. **2.5 млрд** — это количество пользовательских промптов в день. Каждый генерирует один ответ модели.
...
```
DAU ≈ 0.45 × WAU = 0.45 × 800 млн ≈ 360 млн
```

### Разделение по каналам

| Канал | Доля DAU | DAU | Сообщений/день |
|-------|----------|-----|----------------|
| Web | 85% | 306 млн | 2.125 млрд |
| API | 15% | 54 млн | 375 млн |
| **Итого** | 100% | 360 млн | **2.5 млрд** |
...
|----------|-------------|-------------|
| Отправка запроса | 7 | 2.5 млрд / 360 млн DAU |
| Получение ответа (SSE) | 7 | 1 ответ на запрос |

**Вспомогательные (только Web):**

| Действие | Кол-во/день | Обоснование |
|----------|-------------|-------------|
| Загрузка списка чатов | 1 | одна загрузка за сессию |
| Открытие диалога | 3 | переключение между чатами |

**Auth:**

| Действие | Кол-во | Обоснование |
|----------|--------|-------------|
| Auth check | на каждый запрос | проверка куки/токена |
| Login/регистрация | 1/мес | кука/токен живёт 1 месяц |
...
| Датасет / Источник | Период | User (токены) | Assistant (токены) |
|--------------------|--------|---------------|-------------------|
| LMSYS-Chat-1M [^3] | 2023 | ~70 | ~215 |
| WildChat [^4] | 2023–2024 | ~296 | — |
| ShareChat [^5] | 2023–2025 | — | ~1,115 |
| OpenRouter [^6] | 2025 | 6,000 (полный контекст) | ~400 |
| Замер DevTools | 2025 | 1 КБ (~250 токенов) | 8 КБ |

**Ключевое наблюдение**: OpenRouter отражает API-паттерн (большой контекст → короткий ответ). WildChat/ShareChat/DevTools отражают паттерн веб-чата (короткий промпт → развёрнутый ответ).

#### Принятые значения

Замер DevTools: при отправке запроса с одним символом payload составил 0.9 КБ — это фиксированный JSON-overhead HTTP-запроса. Замер ответа модели через токенайзер показал **17.8 байт/токен** для SSE-стрима (JSON-фрейминг `data: {...}\n\n`).
...
**Пояснения:**
- Сетевой payload input = токены × 4 байт + 0.9 КБ JSON-overhead
- Сетевой payload output = токены × 17.8 байт (SSE-фрейминг)
- Для хранения = чистый текст, ~4 байта/токен + 100 байт метаданных (message_id, chat_id, user_id, role, timestamp, token_count)

---

### Средний размер хранилища пользователя (Web)

Среднее количество сообщений на пользователя в месяц:

```
web_messages_per_month = 2.125 млрд/день × 30 / 850 млн MAU_web = 75 сообщений
```
...
| Вложения | 75 × 0.05 × 1 МБ | 3.75 МБ |
| **Итого** | | **~4 МБ/мес** |

Вложения составляют **~94%** хранилища пользователя.

---

## 2.3 Технические метрики

### Размер хранения (весь сервис, за 1 год)

Web MAU = 850 млн. API — stateless, хранилище не требуется.

| Тип данных | Расчёт | Объём/год |
|-----------|--------|-----------|
| Текст (user) | 850М × 12 × 90 КБ | ~0.9 ПБ |
| Текст (assistant) | 850М × 12 × 150 КБ | ~1.5 ПБ |
| Метаданные сообщений | 850М × 12 × 15 КБ | ~0.15 ПБ |
| Вложения | 850М × 12 × 3.75 МБ | **~38.3 ПБ** |
| **Итого** | | **~40.8 ПБ/год** |
...
| Тип | Запросов/день | Avg RPS | Peak RPS (×2) |
|-----|--------------|---------|---------------|
| Web inference | 2.125 млрд | 24,600 | 49,200 |
| API inference | 375 млн | 4,340 | 8,680 |
| **Итого** | **2.5 млрд** | **28,940** | **57,880** |

#### Вспомогательные

| Тип | Запросов/день | Avg RPS | Peak RPS (×2) |
|-----|--------------|---------|---------------|
| Открытие диалога | 918 млн | 10,625 | 21,250 |
| Список чатов | 306 млн | 3,542 | 7,084 |
| Auth check | ~3.7 млрд | 43,100 | 86,200 |
| Login | ~33 млн | 386 | 772 |

---

### Concurrent SSE-соединения

Скорость генерации модели класса Sonnet: **~50 токенов/сек**

| Тип | Output (токены) | Длительность | Avg concurrent | Peak (×2) |
|-----|----------------|-------------|----------------|-----------|
| Web | 500 | 10 сек | 246,000 | 492,000 |
| API | 400 | 8 сек | 34,720 | 69,440 |
| **Итого** | | | **280,720** | **561,440** |
...
| Тип | Расчёт (avg) | Avg | Peak (×2) |
|-----|-------------|-----|-----------|
| Web текст | 24,600 × 2 КБ | 0.38 Гбит/с | 0.77 Гбит/с |
| Web вложения | 24,600 × 0.05 × 1 МБ | 9.84 Гбит/с | 19.69 Гбит/с |
| API текст | 4,340 × 25 КБ | 0.85 Гбит/с | 1.70 Гбит/с |
| **Итого входящий** | | **11.07 Гбит/с** | **22.16 Гбит/с** |

#### Исходящий (сервер → user, SSE)

| Тип | Расчёт (avg) | Avg | Peak (×2) |
|-----|-------------|-----|-----------|
| Web SSE | 24,600 × 9 КБ | 1.72 Гбит/с | 3.45 Гбит/с |
| API SSE | 4,340 × 7 КБ | 0.24 Гбит/с | 0.48 Гбит/с |
| **Итого исходящий** | | **1.96 Гбит/с** | **3.93 Гбит/с** |

#### Суточный трафик

| Тип | Объём/сутки |
|-----|------------|
| Входящий текст (web + API) | ~13.2 ТБ |
| Входящий вложения | ~103.8 ТБ |
| Исходящий SSE | ~19.2 ТБ |
| **Итого** | **~136 ТБ/сутки** |
...
|-------------|---------|----------|
| Web inference | 24,600 | 49,200 |
| API inference | 4,340 | 8,680 |
| Открытие диалога | 10,625 | 21,250 |
| Список чатов | 3,542 | 7,084 |
| Auth check | 43,100 | 86,200 |
| Login | 386 | 772 |
...
[^3]: LMSYS-Chat-1M dataset (2023)

[^4]: WildChat dataset — avg 295.58 user tokens per turn

[^5]: ShareChat dataset — avg 1,115.30 assistant tokens per turn 

[^6]: OpenRouter — avg prompt ~6K tokens, completion ~400 tokens (2025) https://openrouter.ai/state-of-ai

[^7]: DemandSage. *ChatGPT Statistics, User Growth & Usage.* https://www.demandsage.com/chatgpt-statistics/

[^8]: Exploding Topics. *ChatGPT Users Statistics.* https://explodingtopics.com/blog/chatgpt-users
