...
4 ДЦ нужны не для радикального снижения inference TTFT: GPU остаётся в US East / US West, поэтому для EU/APAC сохраняется межрегиональный путь до GPU. Их смысл — локальный HTTP/SSE edge и устойчивость входного слоя:

* локальный TLS/SSE termination ближе к пользователю;
* меньшая задержка для non-inference запросов: login, список чатов, upload, metadata;
* более стабильный вход в сеть через ближайший edge;
* failover HTTP/SSE-слоя при отказе ДЦ;
* разгрузка внешнего Internet path: от ближайшего ДЦ до US GPU трафик идёт по выделенному backbone.
...
| ---------------- | --------------------------------------------------------------------------- |
| TTFT для EU/APAC | Выигрыш ограничен: уменьшается участок client→edge и jitter, но путь до US GPU остаётся |
...
Latency-бюджет для первого токена при централизованном GPU в US:

| Регион пользователя | Клиент → edge | Regional Inference → US GPU | Основной вывод |
| --- | ---: | ---: | --- |
| US East / US West | ~5 мс | ~10 мс | TTFT в основном определяется очередью и prefill на GPU |
